...
| 4 | AI Employees 集成 | 未开始 | 员工可选知识库并通过检索工具回答且显示来源 |
| 5 | NocoBase 与第三方数据源 | 未开始 | 支持 collection、只读 PostgreSQL/MySQL 和 REST 同步 |
...
## 当前迭代：可被 AI 员工实际使用的知识库

| 子项 | 内容 | 状态 |
| --- | --- | --- |
| 向量数据库 | PostgreSQL/pgvector 连接检测、配置记录与列表 | 已完成，待界面验收 |
| 向量存储 | 绑定向量数据库、Embedding 服务商、模型和固定维度 | 已完成，待界面验收 |
| 知识库实体 | 创建、启停、绑定已初始化 Vector store、设置 Top-K 与列表管理 | 已完成，待界面验收 |
| 向量表 | 依据 store 的维度创建 chunk 表、HNSW 索引和版本命名 | 已实现初始化服务，待连接真实服务验收 |
| Embedding 适配 | 复用 AI 插件 LLM 服务配置并调用 Embedding 模型 | 已实现 OpenAI/千问等已注册服务的 provider 复用，待界面触发验收 |
| 资料入库 | 文件/数据源读取、清洗、切片、增量同步、失败重试 | 待开始 |
| AI 检索 | 注册 plugin-ai 知识库 feature、权限过滤、检索引用 | 待开始 |
...
- 实现方式：独立 NocoBase 插件；客户端仅使用 `client-v2`。

## 当前阻塞

- 本机 PATH 中缺少 `node`、`npm`、`corepack` 和 `yarn`，无法按仓库标准执行插件脚手架与构建。
- 无。第 2 阶段开始实现插件内的 Vector database / Vector store 配置与连接检测。

## 2026-07-16 增量：文本入库闭环（进行中）
...
- 下一步：用户在浏览器确认检索命中后，注册并接入 AI Employees 的 knowledge-base feature；随后收敛文本入库接口并开始只读 MySQL/PostgreSQL 数据源连接器。

## 2026-07-16 增量：AI Employees 知识库接入（待对话验收）
...
- 已验证：插件 ESLint 及生产构建通过。
- 下一步：重启后配置一个 AI Employee 的 Knowledge Base 选项，进行真实对话验收；验收成功即完成知识库 MVP，并进入只读 MySQL/PostgreSQL 数据源接入。
...
## 2026-07-16 架构收敛：统一 AI 管理运行时（进行中）
...
- 迁移顺序：知识库/文档管理与检索测试 → 向量存储 → 向量数据库。迁移完成前不再以 iframe 作为方案。
...
## 2026-07-16 增量：多类型文件文本提取（服务端完成，待上传界面）
...
- 待验收：真实文件导入（至少 PDF、DOCX、XLSX、PPTX 各一份）。该项尚未以真实文件完成验收，不能视作已交付完成。
- 已完成：文档级删除。删除时同步清理该文档在 pgvector 中的全部切片，再删除元数据记录，防止已删除内容继续被 AI 检索。
- 下一步：先完成真实文件验收；随后以同一入库接口实现 MySQL/PostgreSQL 只读数据源同步。
